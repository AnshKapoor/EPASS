...
class trial_mat(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.mat_types = ['Isotropic', 'Orthotropic', 'viscoorthotropic', 'Fluid', 'Viscoelastic', 'CLD:Cremer/Heckl',
    ...
    def basic(self):
        ...
        addMat.setIcon(QtGui.QIcon('./materialtab/add_button.png'))
        ...
        self.horiz_layout.addWidget(self.saveMat)
    ...
    def show_mat(self):
        ...
        for i in range(len(self.Mat_Object)):
            ...
            newdelButton.setIcon(QtGui.QIcon('./materialtab/del_button.png'))
    ...
    def saveMatToFile(self,exportbin):
        ...
        with h5py.File(exportbin, 'r+') as binfile:
            ...
            for iMat, Mat in enumerate(self.Mat_Object):
                if (isinstance(Mat, Structural_Isotropic) or isinstance(Mat, Viscoelastic) or isinstance(Mat,                                                                                                          Frequency_Viscoelastic) or Frequency_Viscoelastic_Param or
                        isinstance(Mat, Structural_Orthotropic) or isinstance(Mat,                                                                                  Structural_viscoOrthotropic) or isinstance(
                            Mat, ViscoelasticCLD_CH) or isinstance(Mat, ViscoelasticCLD_RKU) or
                        isinstance(Mat, Fluid) or isinstance(Mat, Fluid_loss) or isinstance(Mat,                                                                                                equivfluid) or isinstance(
                            Mat, Cloaking_fluid) or
                        isinstance(Mat, Poro_Elastic) or isinstance(Mat, spring) or isinstance(Mat, pointmass)):
                        data = Mat.getExportData()
                matList.create_group('mat'+data[2])
