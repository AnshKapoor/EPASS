...
class Structural_Orthotropic():
    
    def __init__(self, Name, Id, Ex, Ey, Ez, Gxy, Gxz, Gyz, nuxy, nuxz, nuyz, rho, t):
        ...
        self.length = 27
        self.type = QtWidgets.QLabel('Orthotropic:')
        self.type.setToolTip('<b>Type of Material</b> <br> Orthotropic linear elastic material to be used with any structural element - without any damping.')
          
    # returns the material parameters and labels to the Parent class-exp_material    
    def getInfo(self):
        return [self.type, QtWidgets.QLabel('Name'), self.name, QtWidgets.QLabel('Id'), self.Id, QtWidgets.QLabel('Ex'), self.Ex, QtWidgets.QLabel('Ey'), self.Ey, QtWidgets.QLabel('Ez'), self.Ez, QtWidgets.QLabel('Gxy'), self.Gxy, QtWidgets.QLabel('Gxz'), self.Gxz, QtWidgets.QLabel('Gyz'), self.Gyz, QtWidgets.QLabel('nuxy'), self.nuxy, QtWidgets.QLabel('nuxz'), self.nuxz, QtWidgets.QLabel('nuyz'), self.nuyz, QtWidgets.QLabel(' ρ'), self.rho, QtWidgets.QLabel('t'), self.t, self.length]
    
    # returns data used for export to myMaterials.txt
    def getExportData(self):
        export_data = ['Orthotropic', self.name.text(), self.Id.text(), self.Ex.text(), self.Ey.text(), self.Ez.text(), self.Gxy.text(), self.Gxz.text(), self.Gyz.text(), self.nuxy.text(), self.nuxz.text(), self.nuyz.text(), self.rho.text(), self.t.text()]
        export_data = [str(item) for item in export_data] # convert QtCore.QString elements to str
        return export_data
    
    # returns the material parameters and labels to the Parent class-exp_material    
    def getParaStudy(self):
        return ['Ex', 'Ey', 'Ez', 'Gxy', 'Gxz', 'Gyz', 'nuxy', 'nuxz', 'nuyz', 'rho', 't']        
