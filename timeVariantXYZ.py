...
# Time variant Load from X,Y,Z data
class timeVar(load):
    ...
    # Clear all content in timeVar layout
    ...
    # Loads X,Y,Z data from json file. Datatype: dict with coord. as tuple as key and list of timestamps as value
    def loadData(filename):
        ...
        for npt, pt in enumerate(keys):
            ptlist = pt[1:-1].split(',')
            keys[npt] = [int(x.strip()) for x in ptlist]
        keystup = [tuple(point) for point in keys]
        ld = dict(zip(keystup, values))
        self.xyzdata = ld
